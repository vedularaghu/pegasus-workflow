...
if [ ! -x seperate ]; then
    /bin/chmod +x seperate
fi

printf "\n##################### Checking file integrity for input files #####################\n"  1>&2
# do file integrity checks
pegasus-integrity --print-timings --verify=stdin 1>&2 << 'eof'
nums4.txt:nums9.txt:nums3.txt:nums10.txt:nums1.txt:seperate:nums2.txt:nums8.txt:nums5.txt:nums6.txt:nums7.txt
eof
...
pegasus-kickstart  -n seperate -N ID0000001 -R condorpool  -s even_nums.txt=even_nums.txt -s odd_nums.txt=odd_nums.txt -L workflow3 -T 2020-07-08T23:14:54+00:00 ./seperate 
job_ec=$?
